...
useful next actions, and discussion has no explicit local continuation surface.
...
subdirective relationship, and archive state. Activity remains outside the
main-chat conversation in a dedicated directive surface with its own local
composer. The execution session stores only compact chronological events that
link to full directive activity and are excluded from conversational context.
...
behavior, and delegated work. Discussion is continued in the directive
activity surface. **Implement this** converts the current directive-local
discussion into a new implementation attempt without importing main-chat
history. After an attempt, **Discuss result** starts or continues a read-only
conversation with that attempt attached.
...
31. As a user, I want the main session to show a compact directive event, so that chronology and Rewind remain understandable without embedding the full response.
32. As a user, I want the compact event to link to the directive activity, so that I can move from session chronology to the full result.
33. As a user, I want discussions displayed in a dedicated directive activity surface, so that long answers do not crowd source overlays or masquerade as main chat.
34. As a user, I want the directive activity surface to have a local composer, so that I can continue a discussion explicitly.
...
68. As a user, I want source-missing directives available in the workspace activity list, so that no live source buffer is required to reach them.
...
- Record a compact non-conversational session event for each directive model
  turn. The event provides ordering, display linkage, and checkpoint lookup but
  is excluded from later model context.
...
- Keep older attempts out of automatic prompt context. They remain available
  for human inspection in the activity surface.
...
- Give directive activity its own workspace surface and local multiline
  composer. Support continuing discussion, Implement this, Discuss result,
  feedback for Request changes, attempt selection for inspection, patch view,
  Rewind entry, reattachment, and Archive from that surface.
...
- Include ordinary chat and all directive events in the same chronological
...
- At the primary seam, assert prompt isolation, fresh reference expansion,
  action-specific context, session binding, compact session events, immutable
  attempt capture, patch ownership, capture completeness, derived lifecycle
  state, and available actions after success, failure, and abort.
...
- Use the real session persistence and Rewind path with temporary files. Create
  ...
  subdirectives, compact-event removal, and state recomputation.
...
- Add small rendering/action tests for attached, detached, source-missing,
  ...
  source seam; substantive content belongs in the activity surface tests.
- Test activity-surface rendering and local composer behavior using existing
  view-zone conventions. Preserve the established invariant that asynchronous
  status or activity redraw does not alter an active multiline composer draft,
  including a draft whose first editable character is `>`.
...
- Remove or rewrite tests that encode Revise, overlay-owned identity,
  directive responses as ordinary main-chat content, shared patch fallback,
  evaporating durable directives, or post-turn-only checkpoint semantics.
...
- Feeding directive activity into main-chat conversational context or moving
  full directive responses into the main chat UI.
