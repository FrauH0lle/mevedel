...
;; any single mevedel module: colour tinting for overlay styling, faces whose
;; colours are blended from the active theme, tag-query prefix/infix
;; conversion, environment-info string assembly for system prompts, synthetic
;; transcript insertion and response-marker handling, and various text and
...
;; A face whose colour must be blended from the active theme cannot express
;; that blend in its `defface' spec, so the spec carries a static fallback and
;; the blended attributes are written here whenever the theme changes.  Each
;; module registers its own faces, so this stays free of view knowledge and a
;; face is derived as soon as the module defining it loads.
