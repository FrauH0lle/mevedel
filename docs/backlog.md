...
### Quiet the deferred diagnostic-flush retry message

- **Source:** `mevedel-telemetry.el` (`mevedel-telemetry-record` error
  echo) with `mevedel-session-publication-append-diagnostic`
- **What's owed:** The idle-transport diagnostic flush sometimes hits
  `append-diagnostic` at a moment the lease cannot be renewed without
  I/O and echoes "telemetry persistence failed: ... could not renew the
  session lease" even though the entries are retained and the next
  flush retries them.  A retained-for-retry outcome should be silent or
  a single deduplicated notice, not a message per attempt.
- **Why deferred:** Behavior is safe (retain + retry, proven by a green
  acceptance journey); only the echo is wrong, and the punch list was
  scoped to correctness.
- **Status check:** Real-remote acceptance journeys print the message
