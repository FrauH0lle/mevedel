...
(mevedel-deftest mevedel-view--path-target ()
  ...
  ;; Decoration runs from a redraw timer.  A stat here lands inside whatever
  ;; remote command the redraw interrupted, and the two cross replies.
  (with-temp-buffer
    (let ((stats 0))
      ;; Redefining a primitive makes Emacs warn about trampolines; the
      ;; stub is the point of the case, so the test owns that warning.
  ...
  ;; Only absence expires; presence is kept for the buffer's life.
  (with-temp-buffer
    (setq mevedel-view--path-existence (make-hash-table :test #'equal))
...
(mevedel-deftest mevedel-view--path-known-p ()
  ...
  :doc "the workspace file cache links a known file with no wait and no I/O"
  ;; Tool output mostly names files mevedel just read or wrote, so the
  ;; common case must not have to wait for the verification pass.
...
(mevedel-deftest mevedel-view--verify-paths ()
  ...
  :doc "verification defers through the transport and memoizes both outcomes"
  (with-temp-buffer
    (let ((buffer (current-buffer))
          deferred-thunk)
      (setq mevedel-view--path-pending
            (list "/mevedelmock:host:/srv/project/gone.py"
                  "/mevedelmock:host:/srv/project/there.py"))
      (mevedel-test--with-captured-diagnostics nil
        (cl-letf (((symbol-function 'mevedel-transport-run-when-idle)
                  ...
                  ((symbol-function 'file-exists-p)
                   (lambda (path &rest _)
                     (string-suffix-p "there.py" path))))
          (mevedel-view--verify-paths buffer)
          ;; Nothing stated yet: the pass waits for an idle transport.
          (should (functionp deferred-thunk))
          (should-not mevedel-view--path-existence)
          (funcall deferred-thunk)
          (should (car (gethash "/mevedelmock:host:/srv/project/there.py"
                                mevedel-view--path-existence)))
          (should-not (car (gethash "/mevedelmock:host:/srv/project/gone.py"
                                    mevedel-view--path-existence))))))))
