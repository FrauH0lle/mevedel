...
(mevedel-deftest mevedel-ediff-patch
  (:doc "Rejects a second mevedel Ediff review before inspecting its buffer")
  ,test
  (test)
  (let ((mevedel--ediff-session '(:token active)))
    (with-temp-buffer
      (let ((failure (should-error (mevedel-ediff-patch) :type 'user-error)))
        (should (string-search "already active"
                               (error-message-string failure)))))))

(mevedel-deftest mevedel--setup-ediff-session
  (:doc "Owns a multi-patch group and forwards startup to child controls")
  ,test
  (test)
  (let* ((group (generate-new-buffer " *mevedel ediff group*"))
         (token (make-symbol "mevedel-ediff-token"))
         (mevedel--ediff-session
          (list :token token :multi-p t :controls nil))
         call)
    (unwind-protect
        (with-current-buffer group
          (setq-local ediff-session-action-function
                      (lambda (file &optional startup-hooks)
                        (setq call (list file startup-hooks))))
          (mevedel--setup-ediff-session token)
          (should (eq group
                      (plist-get mevedel--ediff-session :group-buffer)))
          (should-not (plist-get mevedel--ediff-session :controls))
          (should (local-variable-p 'ediff-quit-session-group-hook))
          (funcall ediff-session-action-function "source.el" (list #'ignore))
          (should (equal "source.el" (car call)))
          (should (= 2 (length (cadr call)))))
      (when (buffer-live-p group)
        (kill-buffer group)))))

(mevedel-deftest mevedel--create-patch-from-ediff
  (:doc "Ignores a quit callback from a foreign Ediff control buffer")
  ,test
  (test)
  (let* ((root (file-name-as-directory
                (make-temp-file "mevedel-ediff-foreign-" t)))
         (first-path (file-name-concat root "first.txt"))
         (second-path (file-name-concat root "second.txt"))
         (first-buffer nil)
         (second-buffer nil)
         (owner (generate-new-buffer " *mevedel ediff owner*"))
         (foreign (generate-new-buffer " *mevedel ediff foreign*"))
         (patch-buffer (generate-new-buffer " *mevedel ediff patch*"))
         (token (make-symbol "mevedel-ediff-token"))
         (mevedel--ediff-session
          (list :token token :controls (list owner)
                :group-buffer owner :patch-buffer patch-buffer)))
    (unwind-protect
        (progn
          (with-temp-file first-path (insert "first\n"))
          (with-temp-file second-path (insert "second\n"))
          (setq first-buffer (find-file-noselect first-path)
                second-buffer (find-file-noselect second-path))
          (with-current-buffer foreign
            (setq-local ediff-buffer-A first-buffer
                        ediff-buffer-B second-buffer)
            (should-not (mevedel--create-patch-from-ediff token)))
          (should (equal "first\n"
                         (with-temp-buffer
          ...
          (should (equal "second\n"
                         (with-temp-buffer
                           ...
                           (buffer-string)))))
      (dolist (buffer (list first-buffer second-buffer owner foreign
                            patch-buffer))
