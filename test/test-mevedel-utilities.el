...
(defface test-mevedel-utilities--face
  '((t :inherit shadow))
  "Throwaway face for the theme-derivation cases."
  :group 'mevedel)

(defun test-mevedel-utilities--hex-rgb (name &optional _frame)
...
(mevedel-deftest mevedel--contrast ()
  ...
  (test-mevedel-utilities--with-gui-colors 16777216
...
(mevedel-deftest mevedel--face-user-styled-p
  (:after-each (progn (put 'test-mevedel-utilities--face 'customized-face nil)
                      (put 'test-mevedel-utilities--face 'theme-face nil)))
  ,test
  (test)
  :doc "ignores a face carrying only its defface spec"
  (should-not (mevedel--face-user-styled-p 'test-mevedel-utilities--face))
  :doc "detects a user customization"
  (progn
    (put 'test-mevedel-utilities--face 'customized-face '((t :height 1.0)))
    (should (mevedel--face-user-styled-p 'test-mevedel-utilities--face)))
  :doc "detects a theme spec"
  (progn
    (put 'test-mevedel-utilities--face 'theme-face '((user ((t :height 1.0)))))
    (should (mevedel--face-user-styled-p 'test-mevedel-utilities--face))))

(mevedel-deftest mevedel--derive-theme-face
  (:vars ((mevedel--theme-derived-faces nil))
   :before-each (set-face-attribute 'default nil
                                    :foreground "#bbc2cf"
                                    :background "#282c34")
   :after-each (progn
                 (put 'test-mevedel-utilities--face 'customized-face nil)
                 (face-spec-set 'test-mevedel-utilities--face
                                '((t :inherit shadow))
                                'face-defface-spec)))
  ,test
  (test)
...
(mevedel-deftest mevedel--derive-theme-faces
  (:vars ((mevedel--theme-derived-faces
           (list (cons 'test-mevedel-utilities--face
  ...
  (test-mevedel-utilities--with-gui-colors 8
    (mevedel--derive-theme-faces)
    (should (eq 'unspecified
                (face-attribute 'test-mevedel-utilities--face
                                :foreground nil)))))
