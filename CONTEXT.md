...
- **session fork** — A child session that continues from a selected assistant response in another session. Its inherited conversation history ends at that fork point, its session settings are copied independently, and its live workflow ownership starts fresh.
- **rewind** — The in-place session undo operation that discards later conversation and transactionally returns planned captured working-tree file contents to an earlier successfully settled turn. Snapshot coverage is limited to state mevedel captured, but an approved Rewind is not partially applied. It creates no session fork or redo variant and does not restore Git index or repository history. _Avoid_: restore
...
- **Plan mode** — A sticky planning conversation in which the user and model create or revise a plan before the user accepts it or explicitly leaves planning. It may use a planning-specific model policy independently of the session's implementation model. Direct file-editing capabilities are withheld and Bash is limited to recognized read-only commands, while Eval and other tools follow ordinary session permissions; Plan mode is toggled independently of permission mode. A session cannot enter Plan while it owns an unfinished Goal.
...
- **goal token budget** — An optional limit on model tokens consumed by Goal turns. It gates new turns but does not interrupt an already-running turn; compaction and child-agent usage are outside the budget. Exhausting it stops automatic continuation without implying completion.
...
- **model workload** — A named kind of model request, such as planning, exploration, verification, or compaction, whose model and reasoning effort can be configured by a preset.
