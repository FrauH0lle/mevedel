...
- **Plan mode** — A sticky planning conversation in which the user and model create or revise a plan before the user accepts it or explicitly leaves planning. It may use a planning-specific model policy independently of the session's implementation model. Direct file-editing capabilities are withheld and Bash is limited to recognized read-only commands, while Eval and other tools follow ordinary session permissions; Plan mode is independent of permission mode even when both are exposed through one UI cycle. A session cannot enter Plan while it owns an unfinished Goal.
